...
const SentOffers = () => {
  ...
  const fetchOffers = async () => {
    try {
      ...
      const buyerDocRef = doc(firestoreDB, "profile", email);
      const querySnapshot = await getDocs(collection(buyerDocRef, "sentOffers"));
...
const OfferItem = ({ item, onPressDetails }) => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        const profileDocRef = doc(firestoreDB, "profile", item.seller);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.contentContainer}>
        ...
        <Text style={styles.offerPrice}>Sent Amount: ${item.offerPrice}</Text>
      </View>
